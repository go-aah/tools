...
func processMount(appBaseDir, vroot, proot string, skipList ess.Excludes, noGzipList []string) error {
	...
	return ioutil.WriteFile(filepath.Join(appBaseDir, "app", filename), b, permRWXRXRX)
}
...
// on file aah.project.
func generateVFSSource(vroot, proot string, skipList ess.Excludes, noGzipList []string) ([]byte, error) {
	...
	buf := &bytes.Buffer{}
	if err = vfsTmpl.ExecuteTemplate(buf, "vfs_start_embed", aah.Data{
		"MountPath":    vroot,
		"PhysicalPath": proot,
	}); err != nil {
		return nil, err
...
const vfsTmplStr = `{{ define "vfs_start_mount"}} // Code generated by aah framework - VFS, DO NOT EDIT.
